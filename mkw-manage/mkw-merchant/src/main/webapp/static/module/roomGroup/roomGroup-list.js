...
layui.use(['table', 'jquery', 'form', 'layer', 'xdialog'], function() {
	...
	table.render({
		id: 'myTable',
	    elem: '#tableContent',
	    url: '/mkw-merchant/roomGroup/list.do',
	    cols: [[
	...
	table.on('tool(tableEvent)', function(obj) {
		if(obj.event === 'edit') {
			openPager('/mkw-merchant/roomGroup/roomGroup-edit.do?id=' + obj.data.id, '编辑');
		} else if(obj.event === 'delete') {
			execute('/mkw-merchant/roomGroup/delete.do', {id: obj.data.id}, function(e) {
				alertify(1, '保存成功');
				flushTable(searchCache, 1);
			});
		} else if(obj.event === 'addFood') {
			openPager('/mkw-merchant/roomGroup/roomGroup-packages.do?id=' + obj.data.id, '添加菜品');
		}
	});
	
	$("#addBtn").click(function() {
		openPager('/mkw-merchant/roomGroup/roomGroup-edit.do', '添加');
	});
