...
layui.use(['table', 'jquery', 'form', 'layer', 'xdialog'], function() {
	...
	table.render({
		id: 'myTable',
	    elem: '#tableContent',
	    url: '/mkw-merchant/roomFood/list.do',
	    cols: [[
	...
	table.on('tool(tableEvent)', function(obj) {
		if(obj.event === 'edit') {
			openPager('/mkw-merchant/roomFood/roomFood-edit.do?id=' + obj.data.id, '编辑');
		} else if(obj.event === 'delete') {
			execute('/mkw-merchant/roomFood/delete.do', {id: obj.data.id}, function(e) {
				alertify(1, '保存成功');
				flushTable(searchCache, 1);
			});
		} else if(obj.event === 'down') {
			execute('/mkw-merchant/roomFood/change.do', {id: obj.data.id, status: 1}, function(e) {
				alertify(1, '保存成功');
				flushTable(searchCache, 1);
			});
		} else if(obj.event === 'up') {
			execute('/mkw-merchant/roomFood/change.do', {id: obj.data.id, status: 0}, function(e) {
				alertify(1, '保存成功');
				flushTable(searchCache, 1);
			});
		}
