...
layui.config({
    ...
    execute('/mkw-client/findMenu.do', {status: 1, level: 1}, function(result) {
        console.log(result);
        //设置navbar
        navbar.set({
            spreadOne: true,
        ...
        navbar.on('click(side)', function (data) {
        	$.each(result.responseData, function(i, item){
        		if(item.title === data.field.title) {
                	...
                	// 打开的新的Tab
                	execute('/mkw-client/findMenu.do', {status: 1, level: 2, parentId: item.id}, function(k) {
        ...
        // 重置第一个tab
        execute('/mkw-client/findMenu.do', {status: 1, level: 2, parentId: result.responseData[0].id}, function(e) {
    ...
    $('#myMessage').click(function() {
        tab.tabAdd({
            href: '/mkw-client/message/list-message.do',
            icon: 'fa-bell-o',
            title: '消息通知'
        });
    });
    ...
    $('#userinfo').click(function() {
        tab.tabAdd({
            href: '/mkw-client/account/edit-userinfo.do',
            icon: 'fa-user-circle',
            title: '个人信息'
        });
    });
    ...
    $('#repass').click(function() {
        tab.tabAdd({
            href: '/mkw-client/account/edit-password.do',
            icon: 'fa-key',
            title: '修改密码'
        });
    });
    ...
    $('#logout').click(function() {
    	top.location.href = "/mkw-client/logout.do";
    });
});