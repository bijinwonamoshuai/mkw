...
layui.use(['table', 'jquery', 'form', 'layer', 'xdialog'], function() {
	...
	table.render({
		id: 'myTable',
	    elem: '#tableContent',
	    url: '/mkw-colligate/rolePermission/list.do',
	    cols: [[
	            {type:'checkbox'},
	...
	table.on('tool(tableEvent)', function(obj) {
		if(obj.event === 'edit') {
			openPager('/mkw-colligate/rolePermission/rolePermission-edit.do?id=' + obj.data.id, '编辑');
		}
	});
	
	$("#addBtn").click(function() {
		openPager('/mkw-colligate/rolePermission/rolePermission-edit.do', '添加');
	});
