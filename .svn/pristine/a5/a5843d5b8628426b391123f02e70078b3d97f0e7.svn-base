...
layui.use(['table', 'jquery', 'form', 'layer', 'xdialog'], function() {
	...
	table.render({
		id: 'myTable',
	    elem: '#tableContent',
	    url: '/mkw-client/room/list.do',
	    cols: [[
	            {type:'checkbox'},
	            {sort: true, field:'name', title: '门店名称'},
	            {sort: true, field:'imageUrl', title: '门店图片图片'},
	...
	table.on('tool(tableEvent)', function(obj) {
		if(obj.event === 'edit') {
			openPager('/mkw-client/room/room-edit.do?id=' + obj.data.id, '编辑');
		}
	});
	
	$("#addBtn").click(function() {
		openPager('/mkw-client/room/room-add.do', '添加');
	});
	...
	$("#searchBtn").click(function() {
		xdialog.init({
			title: '查询',
			columns: [
				{ type: 'text', name: 'name', label: '菜品名称' }
			],
			buttons: [
